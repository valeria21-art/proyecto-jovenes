...
export const UserSettings = ({ user, setUser }: UserProps) => {
  ...
  useEffect(() => {
    document.title = `List of the day - User ${user.name ? `(${user.name})` : ""}`;
  }, [user.name]);
  ...
  const handleSaveName = () => {
    ...
    toast.success(() => (
      <div>
        Se cambió el nombre de usuario a - <b>{name}</b>.
      </div>
    ));
    setName("");
  };
  ...
  return (
    <>
      ...
      <Container>
        ...
        <Tooltip title={user.profilePicture ? "Cambiar foto de perfil" : "Agregar foto de perfil"}>
          <Badge
            overlap="circular"
        ...
        >
          <CreatedAtDate>
          Registrado desde {new Date(user.createdAt).toLocaleDateString()}
          </CreatedAtDate>
        </Tooltip>

        <StyledInput
          label={user.name === null ? "Agregar nombre" : "Cambiar nombre"}
          value={name}
          ...
          helperText={
            name.length > USER_NAME_MAX_LENGTH
              ? `El nombre es demasiado largo como máximo ${USER_NAME_MAX_LENGTH} caracteres`
              : ""
          }
        />
          ...
          <StyledInput
            ...
            helperText={
              profilePictureURL.length > PROFILE_PICTURE_MAX_LENGTH
                ? `La URL es demasiado larga como máximo ${PROFILE_PICTURE_MAX_LENGTH} caracteres`
                : ""
            }
            onChange={(e) => {
      ...
      >
        ...
        <DialogContent>
        ¿Estás segura de que quieres cerrar sesión? <b>Tus tareas no se guardarán.</b>
        </DialogContent>
        <DialogActions>
