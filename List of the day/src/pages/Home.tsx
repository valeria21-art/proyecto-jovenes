...
export const Home = ({ user, setUser }: UserProps) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRandomGreeting(getRandomGreeting());
      setGreetingKey((prevKey) => prevKey + 1); //Actualiza la clave en cada intervalo
    }, 6000);
      ...
      {user.tasks.length > 0 && (
        <TasksCountContainer>
          <TasksCount glow={user.settings[0].enableGlow}>
            ...
            <TaskCountTextContainer>
              <TaskCountHeader>
              Tienes {user.tasks.length - completedTasksCount} tareas inacabadas{" "}
              ...
              {tasksWithDeadlineTodayCount > 0 && (
                <span>Tareas pendientes hoy: {tasksWithDeadlineTodayCount}</span>
              )}
            </TaskCountTextContainer>
          </TasksCount>
        </TasksCountContainer>
