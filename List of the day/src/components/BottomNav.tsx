...
/**
 * Componente para renderizar la barra de navegación inferior.
 */
export const BottomNav = ({ user }: BottomNavProps): JSX.Element | null => {
  ...
  // UseEffect gancho para configurar el botón activo según la ruta actual
  useEffect(() => {
    const pathParts = location.pathname.split("/"); // Divida el nombre de la ruta por'/'
    if (pathParts[1] === "task") {
      setValue(0); //Si el usuario está en una página de tarea, establezca el valor en 0
    } else {
     // Maneja otras rutas como antes
